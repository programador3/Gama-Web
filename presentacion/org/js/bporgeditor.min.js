...
primitives.orgeditor.UserTemplateDescription.prototype.onRender = function (a, c, b) {
    ...
    ["title", "description", "readmorelabel"]; b = 0; for (d = e.length; b < d; b += 1) f = e[b], g = c.element.find("[name=" + f + "]"), f = null != a[f] ? a[f] : "", g.text() !== f && g.text(f); h.css({ visibility: !primitives.common.isNullOrEmpty(a.readmorelabel) ? "inherit" : "hidden" }); primitives.common.isNullOrEmpty(a.readmoreurl) ? h.removeAttr("href") : h.attr({ href: a.readmoreurl })
}; primitives.orgeditor.UserTemplateInvisible = function () { this.name = "invisibleTemplate" }; primitives.orgeditor.UserTemplateInvisible.prototype = new primitives.orgeditor.UserTemplate;
primitives.orgeditor.UserTemplateInvisible.prototype.getTemplate = function () {
    var a = new primitives.orgdiagram.TemplateConfig, c; a.name = this.name; a.itemSize = new primitives.common.Size(120, 100); a.minimizedItemSize = new primitives.common.Size(4, 4); a.highlightPadding = new primitives.common.Thickness(2, 2, 2, 2); c = jQuery('<div class="bp-item bp-corner-all bt-item-frame"><div class="bp-item" style="top: 4px; left: 4px; width: 112px; height: 92px; font-size: 10px;">This item is invisible to user and its children logically belong to its first visible parent.</div></div>').css({
        width: a.itemSize.width +
        "px", height: a.itemSize.height + "px"
    }); a.itemTemplate = c.wrap("<div>").parent().html(); return a
}; primitives.orgeditor.UserTemplateInvisible.prototype.onRender = function (a, c) { var b = c.context; c.element.find("[name=title]").text(b.title) };
primitives.orgeditor.Config = function () {
    this.classPrefix = "bporgeditor"; this.onSave = null; this.editMode = !0; this.pageFitMode = primitives.common.PageFitMode.FitToPage; this.verticalAlignment = primitives.common.VerticalAlignmentType.Middle; this.horizontalAlignment = primitives.common.HorizontalAlignmentType.Center; this.connectorType = primitives.common.ConnectorType.Angular; this.items = null; this.selectionPathMode = primitives.common.SelectionPathMode.FullStack; this.minimalVisibility = primitives.common.Visibility.Dot;
...
primitives.orgeditor.Controller.prototype._onJsonButtonClick = function () {
    var a = this, c, b, d, e; if (null != this.dlgCodeMirror) {
        a.dlgCodeMirror.bpDlgCodeMirror("option", { cancel: function () { }, update: function (f, g) { e = {}; c = 0; for (b = a.fieldsToClone.length; c < b; c += 1) d = a.fieldsToClone[c], a.options[d] = g[d], e[d] = g[d]; a._updateItemConfigs(e.items); e.cursorItem = null != e.items[0] ? e.items[0].id : null; a.orgDiagram.orgDiagram("option", e); a.orgDiagram.orgDiagram("update"); a._trigger("onSave") } }); e = {}; c = 0; for (b = a.fieldsToClone.length; c <
        b; c += 1) d = a.fieldsToClone[c], e[d] = a.options[d]; a.dlgCodeMirror.bpDlgCodeMirror("open", JSON.stringify(e, void 0, 2))
    } else alert("In order to enable this function you have to download latest version of Basic Primitives Joomla modules.")
}; primitives.orgeditor.Controller.prototype._cleanLayout = function () { this.element.find("." + this.widgetEventPrefix).remove() };
...
primitives.orgeditor.Controller.prototype._getOrgDiagramConfig = function () {
    ...
    new primitives.orgdiagram.ButtonConfig("move", "ui-icon-arrow-4", "Move")], f.onButtonClick = function (b, c) { a._onButtonClick(b, c) }); return f
}; primitives.orgeditor.Controller.prototype._onMouseClick = function (a, c) { if (!this.options.editMode) { var b = this.orgDiagram.orgDiagram("option", "cursorItem"); null != c.context && (b === c.context.id && !primitives.common.isNullOrEmpty(b.readmoreurl)) && (window.location.href = b.readmoreurl) } };
primitives.orgeditor.Controller.prototype._onButtonClick = function (a, c) {
    var b = this, d, e, f, g, h, i, j; switch (c.name) {
        case "delete": null === c.parentItem ? b.dlgMessage.dialog({ modal: !0, buttons: { Ok: function () { jQuery(this).dialog("close") } } }) : b.dlgConfirm.dialog({
            resizable: !1, height: 240, modal: !0, buttons: {
                Delete: function () {
                    j = b._getSubItemsForParent(c.context); j[c.context.id] = !0; i = []; e = 0; for (f = b.options.items.length; e < f; e += 1) h = b.options.items[e], j.hasOwnProperty(h.id) || i.push(h); b.options.items = i; b.orgDiagram.orgDiagram("option",
                    { items: b.options.items, cursorItem: c.context.parent }); b.orgDiagram.orgDiagram("update", primitives.common.UpdateMode.Refresh); jQuery(this).dialog("close"); b._trigger("onSave")
                }, Cancel: function () { jQuery(this).dialog("close") }
            }
        }); break; case "properties": b.dlgItemConfig.bpDlgItemConfig("option", {
            cancel: function () { }, update: function () {
                var a = b.dlgItemConfig.bpDlgItemConfig("option", "children"), c, d = {}, f; e = 0; for (c = a.length; e < c; e += 1) f = a[e], d[f.id] = !0; i = []; e = 0; for (c = b.options.items.length; e < c; e += 1) f = b.options.items[e],
                d.hasOwnProperty(f.id) || i.push(f); b.options.items = i.concat(a); b.orgDiagram.orgDiagram("option", { items: b.options.items }); b.orgDiagram.orgDiagram("update", primitives.common.UpdateMode.Refresh); b._trigger("onSave")
            }, itemConfig: c.context, children: b._getChildrenForParent(c.context)
        }); b.dlgItemConfig.bpDlgItemConfig("open"); break; case "add": b.selectedItem = c.context; b.dlgItemConfig.bpDlgItemConfig("option", {
            cancel: function () { }, update: function () {
                var a = b.dlgItemConfig.bpDlgItemConfig("option", "itemConfig");
                a.id = b._getMaximumId() + 1; a.parent = b.selectedItem.id; b.options.items.push(a); b.orgDiagram.orgDiagram("option", { items: b.options.items, cursorItem: a.id }); b.orgDiagram.orgDiagram("update", primitives.common.UpdateMode.Refresh); b._trigger("onSave")
            }, itemConfig: null, children: []
        }); b.dlgItemConfig.bpDlgItemConfig("open"); break; case "move": if (null === c.parentItem) b.dlgMessage.dialog({ modal: !0, buttons: { Ok: function () { jQuery(this).dialog("close") } } }); else {
            b.selectedItem = c.context; b.dlgOrgChart.bpDlgOrgDiagram("option",
...
primitives.orgeditor.DlgCodeMirror.prototype.open = function (a) {
    var c = this; this.content.val(a); this.editor = CodeMirror.fromTextArea(this.content[0], { lineNumbers: !0, matchBrackets: !0 }); this.element.dialog({
        autoOpen: !1, minWidth: 640, minHeight: 480, modal: !0, title: "Organizational Chart Data", buttons: { Save: function () { a = JSON.parse(c.editor.getValue()); jQuery(this).dialog("close"); c._trigger("update", null, a) }, Cancel: function () { jQuery(this).dialog("close"); c._trigger("cancel", null) } }, resizeStop: function () { c._updateLayout() },
        close: function () { c.editor.toTextArea(); c._trigger("cancel", null) }, open: function () { c._updateLayout() }
    }).dialog("open")
}; primitives.orgeditor.DlgCodeMirror.prototype._setOption = function (a, c) { jQuery.Widget.prototype._setOption.apply(this, arguments) }; (function (a) { a.widget("ui.bpDlgCodeMirror", new primitives.orgeditor.DlgCodeMirror) })(jQuery); primitives.orgeditor.DlgConfigOptions = function () { this.update = this.cancel = null };
primitives.orgeditor.DlgConfig = function () {
    this.widgetEventPrefix = "bpdlgconfig"; this.options = new primitives.orgeditor.DlgConfigOptions; this.tips = this.itemTitleSecondFontColor = this.itemTitleFirstFontColor = this.labelHeight = this.labelWidth = this.labelPlacement = this.labelOrientation = this.showLabels = this.defaultTemplateName = this.leavesPlacementType = this.childrenPlacementType = this.selectionPathMode = this.minimalVisibility = this.connectorType = this.horizontalAlignment = this.verticalAlignment = this.orientationType =
    this.pageFitMode = null; this.enums = {
        pageFitMode: { name: primitives.common.PageFitMode, title: "Page Fit Mode", isString: !1 }, orientationType: { name: primitives.common.OrientationType, title: "Orientation", isString: !1 }, verticalAlignment: { name: primitives.common.VerticalAlignmentType, title: "Items Vertical Alignment", isString: !1 }, horizontalAlignment: { name: primitives.common.HorizontalAlignmentType, title: "Items Horizontal Alignment", isString: !1 }, connectorType: {
            name: primitives.common.ConnectorType, title: "Connectors",
            isString: !1
        }, minimalVisibility: { name: primitives.common.Visibility, title: "Minimal nodes visibility", isString: !1 }, selectionPathMode: { name: primitives.common.SelectionPathMode, title: "Selection Path Mode", isString: !1 }, childrenPlacementType: { name: primitives.common.ChildrenPlacementType, title: "Children placement", isString: !1 }, leavesPlacementType: { name: primitives.common.ChildrenPlacementType, title: "Leaves placement", isString: !1 }, defaultTemplateName: {
            name: primitives.orgeditor.TemplateName, title: "Default item template",
            isString: !0
        }, showLabels: { name: primitives.common.Enabled, title: "Show labels", isString: !1 }, labelOrientation: { name: primitives.text.TextOrientationType, title: "Labels orientation", isString: !1 }, labelPlacement: { name: primitives.common.PlacementType, title: "Labels placement", isString: !1 }
    }
}; primitives.orgeditor.DlgConfig.prototype._create = function () { this._createLayout() }; primitives.orgeditor.DlgConfig.prototype.destroy = function () { this._cleanLayout() };
primitives.orgeditor.DlgConfig.prototype._createLayout = function () {
    var a, c, b; a = '<p class="validateTips">All form fields are required.</p><form><fieldset>'; for (c in this.enums) this.enums.hasOwnProperty(c) && (b = this.enums[c], a += '<br/><label for="' + c + '">' + b.title + '</label><select class="text ui-widget-content ui-corner-all" style="padding:2px; margin:5px;" name="' + c + '"></select>'); a = jQuery(a + '<br/><label for="labelWidth">Label width</label><input type="text" name="labelWidth" class="text ui-widget-content ui-corner-all" /><br/><label for="labelHeight">Label height</label><input type="text" name="labelHeight" class="text ui-widget-content ui-corner-all" /><br/><label for="itemTitleFirstFontColor">Title first font color</label><input type="text" name="itemTitleFirstFontColor" class="text ui-widget-content ui-corner-all" /><br/><label for="itemTitleSecondFontColor">Title second font color</label><input type="text" name="itemTitleSecondFontColor" class="text ui-widget-content ui-corner-all" /></fieldset></form>').addClass(this.widgetEventPrefix);
...
primitives.orgeditor.DlgConfig.prototype._createWidgets = function () {
    ...
    this.itemTitleSecondFontColor.autocomplete({ source: d }); for (a in this.enums) if (this.enums.hasOwnProperty(a)) { c = this.enums[a]; this[a] = this.element.find("[name=" + a + "]"); for (b in c.name) c.name.hasOwnProperty(b) && (d = c.name[b], this[a].append(jQuery("<option value='" + d + "'>" + b + "</option>"))); this[a].buttonset() }
}; primitives.orgeditor.DlgConfig.prototype._cleanLayout = function () { this.element.empty(); this.element.removeClass("dialog-form") };
primitives.orgeditor.DlgConfig.prototype.open = function (a) {
    var c = jQuery([]).add(this.itemTitleFirstFontColor).add(this.itemTitleSecondFontColor), b = this, a = void 0 !== a ? a : new primitives.orgdiagram.ItemConfig; this._updateWidgets(a); this.element.dialog({
        autoOpen: !1, height: 400, width: 350, modal: !0, title: "Chart options", buttons: {
            Save: function () {
                var d = !0; c.removeClass("ui-state-error"); if (d = (d = (d = (d = (d = (d = d && primitives.orgeditor.checkLength(b.tips, b.itemTitleFirstFontColor, "Title first font color", 0, 20)) && primitives.orgeditor.checkLength(b.tips,
        ...
        }, close: function () { c.val("").removeClass("ui-state-error"); b._trigger("cancel", null, a) }
    }).dialog("open")
};
...
primitives.orgeditor.DlgItemConfig = function () {
    this.widgetEventPrefix = "bpdlgitemconfig"; this.options = new primitives.orgeditor.DlgItemConfigOptions; this.defaultTemplateName = this.childrenPlacementType = this.adviserPlacementType = this.itemType = this.labelHeight = this.labelWidth = this.labelPlacement = this.labelOrientation = this.showLabels = this.sortableList = this.groupTitleColor = this.groupTitle = this.readmoreurl = this.readmorelabel = this.image = this.email = this.phone = this.description = this.itemTitleColor = this.title =
    null; this.textFields = { title: { title: "Title" }, itemTitleColor: { title: "Title Color" }, description: { title: "Description" }, phone: { title: "Phone" }, email: { title: "E-mail" }, groupTitle: { title: "Group Title" }, groupTitleColor: { title: "Group Title Color" }, image: { title: "Photo" }, readmorelabel: { title: "Read more label" }, readmoreurl: { title: "Read more URL" }, label: { title: "Label" } }; this.tips = null; this.enums = {
        itemType: {
            name: { Regular: 0, Assistant: 1, Adviser: 2, SubAssistant: 4, SubAdviser: 5, GeneralPartner: 6, LimitedPartner: 7, AdviserPartner: 8 },
            title: "Item type", isString: !1
        }, adviserPlacementType: { name: primitives.common.AdviserPlacementType, title: "Adviser placement", isString: !1 }, childrenPlacementType: { name: primitives.common.ChildrenPlacementType, title: "Children placement", isString: !1 }, templateName: { name: primitives.orgeditor.TemplateName, title: "Item template", isString: !0 }, showLabel: { name: primitives.common.Enabled, title: "Show labels", isString: !1 }, labelOrientation: { name: primitives.text.TextOrientationType, title: "Labels orientation", isString: !1 },
...
primitives.orgeditor.DlgItemConfig.prototype._createWidgets = function () {
    var a = [], c, b, d, e, f; this.element.tabs(); this.sortableList = this.element.find("[name=sortable]"); this.sortableList.sortable(); this.sortableList.disableSelection(); for (b in this.textFields) this.textFields.hasOwnProperty(b) && (this[b] = this.element.find("[name=" + b + "]")); this.tips = this.element.find(".validateTips"); for (e in this.enums) if (this.enums.hasOwnProperty(e)) {
        f = this.enums[e]; this[e] = this.element.find("[name=" + e + "]"); for (b in f.name) f.name.hasOwnProperty(b) &&
        (d = f.name[b], this[e].append(jQuery("<option value='" + d + "'>" + b + "</option>"))); this[e].buttonset()
    } for (c in primitives.common.Colors) primitives.common.Colors.hasOwnProperty(c) && a.push(c); this.itemTitleColor.autocomplete({ source: a }); this.groupTitleColor.autocomplete({ source: a }); this.labelWidth = this.element.find("[name=labelWidth]"); this.labelHeight = this.element.find("[name=labelHeight]")
...
primitives.orgeditor.DlgItemConfig.prototype.open = function () {
    var a = this; this.options.itemConfig = null != this.options.itemConfig ? this.options.itemConfig : new primitives.orgdiagram.ItemConfig; this._updateWidgets(this.options.itemConfig, this.options.children); this.element.dialog({
        autoOpen: !1, height: 400, width: 350, modal: !0, title: "Item options", buttons: {
            Save: function () {
                var c = !0, b; for (b in a.textFields) a.textFields.hasOwnProperty(b) && a[b].removeClass("ui-state-error"); if (c = (c = (c = (c = (c = (c = (c = (c = c && primitives.orgeditor.checkLength(a.tips,
            ...
            }, Cancel: function () { jQuery(this).dialog("close"); a._trigger("cancel", null) }
        }, close: function () { for (var c in a.textFields) a.textFields.hasOwnProperty(c) && a[c].val("").removeClass("ui-state-error"); a._trigger("cancel", null) }
    }).dialog("open")
};
primitives.orgeditor.DlgItemConfig.prototype._updateWidgets = function (a, c) {
    var b, d, e; for (d in this.textFields) this.textFields.hasOwnProperty(d) && this[d].val(a[d]); for (b in this.enums) this.enums.hasOwnProperty(b) && (d = a[b], null == d && (d = this._getFirstEnumItem(this.enums[b].name)), this[b].val(d)); this.sortableList.empty(); b = 0; for (d = c.length; b < d; b += 1) e = c[b], this.sortableList.append(jQuery('<li id="' + b + '" class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>' + e.title + "</li>")); this.labelWidth.val(null !=
    a.labelSize ? a.labelSize.width : ""); this.labelHeight.val(null != a.labelSize ? a.labelSize.height : "")
}; primitives.orgeditor.DlgItemConfig.prototype._getFirstEnumItem = function (a) { for (var c in a) if (a.hasOwnProperty(c)) return a[c]; return null };
primitives.orgeditor.DlgItemConfig.prototype._updateItemConfig = function () {
    var a, c, b, d; for (c in this.textFields) this.textFields.hasOwnProperty(c) && (this.options.itemConfig[c] = this[c].val()); for (a in this.enums) this.enums.hasOwnProperty(a) && (c = this[a].val(), this.options.itemConfig[a] = this.enums[a].isString ? c : parseInt(c, 10)); b = this.sortableList.sortable("toArray"); if (0 < b.length) {
        d = []; a = 0; for (c = b.length; a < c; a += 1) d.push(this.options.children[parseInt(b[a], 10)]); this.options.children.length = 0; this.options.children =
        d
    } a = parseFloat(this.labelWidth.val()); c = parseFloat(this.labelHeight.val()); this.options.itemConfig.labelSize = 0 < a && 0 < c ? new primitives.common.Size(a, c) : null
}; primitives.orgeditor.DlgItemConfig.prototype._setOption = function (a, c) { jQuery.Widget.prototype._setOption.apply(this, arguments) }; (function (a) { a.widget("ui.bpDlgItemConfig", new primitives.orgeditor.DlgItemConfig) })(jQuery); primitives.orgeditor.DlgOrgDiagramOptions = function () { this.update = this.cancel = null };
...
primitives.orgeditor.DlgOrgDiagram.prototype.open = function (a, c) {
    var b = jQuery([]), d = null, e = this, a = void 0 !== a ? a : new primitives.orgdiagram.Config; this.element.dialog({
        autoOpen: !1, minWidth: 640, minHeight: 480, modal: !0, title: "Select new parent", buttons: { Select: function () { var a = !0; b.removeClass("ui-state-error"); d = e.orgdiagram.orgDiagram("option", "cursorItem"); if (a = !e._isParentOf(c, d)) jQuery(this).dialog("close"), e._trigger("update", null, d) }, Cancel: function () { jQuery(this).dialog("close"); e._trigger("cancel") } },
        close: function () { b.val("").removeClass("ui-state-error"); e._trigger("cancel") }, resizeStop: function () { e._updateLayout(); e.orgdiagram.orgDiagram("update", primitives.common.UpdateMode.Refresh) }, open: function () { e._updateWidgets(a) }
    }).dialog("open")
}; primitives.orgeditor.DlgOrgDiagram.prototype._isParentOf = function (a, c) { var b = !1; if (a.id == c.id) b = !0; else for (; null != c.parent;) if (c = this._items[c.parent], c.id == a.id) { b = !0; break } return b };
