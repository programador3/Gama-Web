...
{
    public partial class pre_empleados_captura_basica : System.Web.UI.Page
    {
        public string ruta = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (Request.QueryString["idc_pre_empleado"] != null)
                {
                    CargarInformaciónCandidato(Convert.ToInt32(funciones.de64aTexto(Request.QueryString["idc_pre_empleado"])));
                }
            }
        }
        ...
        public void CargaInformacion(int idc_candidato)
        {
            try
            {
                ...
                DataSet ds = componente.CargaInformacionBasica(entidad);
        ...
        /// <summary>
        /// Carga la informacion ya captura del candidato
        /// </summary>
        /// <param name="idc_pre_Empleado"></param>
        private void CargarInformaciónCandidato(int idc_pre_Empleado)
        {
            try
            {
                Pre_EmpleadosCOM componente = new Pre_EmpleadosCOM();
                DataSet ds = componente.sp_informacion_solicitud_preempleados(idc_pre_Empleado);
                DataTable dt_informacion_personal = ds.Tables[0];
                DataTable dt_Telefonos = ds.Tables[1];
                DataTable dt_hijos = ds.Tables[2];
                DataTable dt_referencias = ds.Tables[4];
                DataTable dt_papeleria = ds.Tables[3];
                if (dt_informacion_personal.Rows.Count > 0)
                {
                    string ruta_foto = dt_informacion_personal.Rows[0]["ruta_foto"].ToString().Trim().ToUpper();
                    if (File.Exists(ruta_foto))
                    {
                        ...
                        string file_name = Path.GetFileName(ruta_foto); ;
                        string file_Dest = dirInfo.ToString() + file_name;
                        File.Copy(ruta_foto, file_Dest, true);
                        imgpreempleado.ImageUrl = System.Configuration.ConfigurationManager.AppSettings["server"] + "temp/pre_alta/personal/" + file_name + "?id=" + randomNumber.ToString().Trim();
                        DataTable foto = Session["foto"] as DataTable;
                        DataRow new_row = foto.NewRow();
                        new_row["nombre"] = file_name;
                        new_row["extension"] = Path.GetExtension(file_name);
                        new_row["ruta"] = file_Dest;
                        foto.Rows.Add(new_row);
                        Session["foto"] = foto;
                    ...
                    txtNombres.Text = dt_informacion_personal.Rows[0]["NOMBRES"].ToString().Trim().ToUpper();
                    txtPaterno.Text = dt_informacion_personal.Rows[0]["PATERNO"].ToString().Trim().ToUpper();
                    txtMaterno.Text = dt_informacion_personal.Rows[0]["MATERNO"].ToString().Trim().ToUpper();
                     txtCorreo.Text = dt_informacion_personal.Rows[0]["correo_personal"].ToString().Trim().ToUpper();
                    txtFecNac.Text = Convert.ToDateTime(dt_informacion_personal.Rows[0]["FEC_NAC"]).ToString("yyyy-MM-dd");
                    ddlTitulo.SelectedValue = dt_informacion_personal.Rows[0]["TITULO"].ToString().Trim().ToUpper();
                    ddlSexo.SelectedValue = dt_informacion_personal.Rows[0]["SEXO"].ToString().Trim().ToUpper();
                    foreach (DataRow row in dt_Telefonos.Rows)
                    {
                        string telefono = row["telefono"].ToString().Trim().ToUpper();
                        string tipo = row["descripcion"].ToString().Trim().ToUpper();
                        AddTelefono(telefono, tipo);
                    }
                    
                    foreach (DataRow row in dt_papeleria.Rows)
                    {
                        string idc_tipodoc = row["idc_tipodoc"].ToString().Trim().ToUpper();
                        string idc_tipodocarc = row["idc_tipodocarc"].ToString().Trim().ToUpper();
                        string archivo = row["archivo"].ToString().Trim().ToUpper();
                        string ruta = row["ruta"].ToString().Trim().ToUpper();
                        string descripcion = row["descripcion"].ToString().Trim().ToUpper();
                        string filename = Path.GetFileName(ruta);
                        string extension = Path.GetExtension(ruta);
                        AddPapeleriaToTable(idc_tipodocarc, idc_tipodoc, descripcion, ruta, filename, extension);

                    }
                }
                else
                {
                    Alert.ShowAlertError("NO SE ENCONTRO LA INFORMACIÓN.", this);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Agrega un Telefono a la tabla de telefonos
        /// </summary>
        /// <param name="telefono"></param>
        /// <param name="tipo"></param>
        private bool AddTelefono(string telefono, string tipo)
        {
            bool retur = true;
            try
            {
                ...
                int numbers = telefono.Length;
                if (telefono == "" || numbers < 8)
                {
                    Alert.ShowAlertError("ESCRIBA EL TELEFONO A 8 o 10 DIGITOS", this);
                    retur = false;
                }
                else
                {
                    ...
                    {
                        retur = false;
                        Alert.ShowAlertError("YA EXISTE ESTE TELEFONO", this);
                    }
                }
                return retur;
            }
        ...
        public bool CopiarArchivos(string sourcefilename, string destfilename)
        {
            try
            {
                if (sourcefilename.ToUpper().Trim() == destfilename.ToUpper().Trim())
                {
                    return true;
                }
                else {
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (txtNombres.Text == "" || txtPaterno.Text == "" || txtMaterno.Text == "" || ddlSexo.SelectedValue == "0" ||ddlTitulo.SelectedValue=="0" || txtFecNac.Text=="")
            {
                Alert.ShowAlertError("INGRESE TODA LA INFORMACION PERSONAL", this);
            }
            else if (TotalCadenaTelefonos() == 0)
            {
                Alert.ShowAlertError("AL MENOS UN TELEFONO DE CONTACTO ES OBLIGATORIO", this);
            }
            else if (papeleria_det.Rows.Count == 0)
            {
                Alert.ShowAlertError("FALTA PAPELERIA", this);
            }
            else
            {
            ...
            try
            {
                ...
                switch (Confirma_a)
                {
                    case "Guardar":
                        ...
                        if (Request.QueryString["idc_pre_empleado"] != null)
                        {
                            entidad.Idc_pre_empleado = Convert.ToInt32(funciones.de64aTexto(Request.QueryString["idc_pre_empleado"]));
                            ds = componente.EditarPreEmpleas(entidad);
                        }
                        else {
                            ds = componente.GuardarPreEmpleas(entidad);
                        }
